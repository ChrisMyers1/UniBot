...
import { ensureGuild } from "../lib/db";
...
export const data = new SlashCommandBuilder()
  .setName("removetime")
  .setDescription("Remove time from a user's current-week total (admin only)")
  .addUserOption((opt) =>
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  await interaction.reply(
    `➖ Removed ${formatDuration(seconds)} from <@${target.id}>'s current week total.`
  );
}
